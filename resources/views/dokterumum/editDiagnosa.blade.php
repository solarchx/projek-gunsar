...
<head>
    ...
    <title>Edit Diagnosa Poli Umum</title>
...
<body>
    <div class="d-flex flex-column min-vh-100">
        ...
        <div class="header d-flex justify-content-between align-items-center">
            <h3 class="mb-0"><i class="fas fa-stethoscope me-2"></i> Edit Diagnosa Poli Umum</h3>
        ...
        <div class="flex-grow-1 container py-4">
            <div class="card-form mx-auto">
                <form class="w-100" method="POST" action="{{ route('rekam-medis.update', $rekamMedis->id) }}">
                    @csrf
                    @method('PUT')

                    <input type="hidden" name="NIK_pasien" value="{{ $rekamMedis->NIK_pasien }}">
                    <input type="hidden" name="NIP_dokter" value="{{ $rekamMedis->NIP_dokter }}">
                    ...
                    <!-- Isi disini untuk data screening -->
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" rows="2" name="keluhan">{{ old('keluhan', $rekamMedis->keluhan) }}</textarea>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Riwayat Penyakit</label>
                        <textarea class="form-control" rows="2" name="riwayat_penyakit">{{ old('riwayat_penyakit', $rekamMedis->riwayat_penyakit) }}</textarea>
                    </div>

                    <!-- Pemeriksaan Fisik (Output Only) sama disini -->
                    <div class="section-title">
                    ...
                    <div class="mb-3">
                        ...
                        <select id="penyakit" name="id_penyakit" class="form-select" onchange="showDiagnosaLainnya()">
                            ...
                            <option value="{{ $penyakit->id }}" {{ old('id_penyakit', $rekamMedis->id_penyakit) == $penyakit->id ? 'selected' : '' }}>
                                {{ $penyakit->nama }}
                            </option>
                            @endforeach
                            <option value="lainnya" {{ old('id_penyakit', $rekamMedis->id_penyakit) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                    </div>

                    <div class="mb-3 {{ old('id_penyakit', $rekamMedis->id_penyakit) == 'lainnya' ? '' : 'd-none' }}" id="diagnosaLainnya">
                        <label class="form-label">Diagnosa Lainnya</label>
                        <input type="text" name="penyakit_baru" class="form-control"
                            value="{{ old('penyakit_baru') }}">
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Catatan</label>
                        <input type="text" name="catatan" class="form-control" value="{{ old('catatan', $rekamMedis->catatan) }}">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="terapi_tindakan" rows="3">{{ old('terapi_tindakan', $rekamMedis->terapi_tindakan) }}</textarea>
                    </div>

                    <!-- Tombol -->
                    <div class="mt-4 action-buttons">
                        <button type="submit" class="btn btn-success flex-fill">
                            <i class="fas fa-save me-1"></i> Update Diagnosa
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        function showDiagnosaLainnya() {
            const selectDiagnosa = document.querySelector('#penyakit');
